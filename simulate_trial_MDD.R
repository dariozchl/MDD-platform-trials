  ...
  while (!trial_stop) {
    ...
    if(timestamp %% 4 == 0){
      ...
      if(rand_type == "block_1"){
        ...
        if(patients_per_timepoint[1]*weeks_left >= n_needed + n_fin[[1]]*0.2/frac_to_treatment){
          ok_to_add <- TRUE
        }
      ...
      #only check for new compounds, if at least 20% of the required patients can be recruited before month 60
