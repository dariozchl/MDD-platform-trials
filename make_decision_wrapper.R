make_decision_wrapper <- function(res_list, p_val_interim, p_val_final, n_fin, n_int) { 
    ...
    for(i in 1:length(ways_of_administration)){
      ...
      for(j in arms_within_administration){
        if(!is.null(res_list[[population]][[j]]$data)){
          if(nrow(res_list[[population]][[j]]$data) > n_fin & res_list[[population]][[j]]$decision[2] == "none"){
            output <- make_decision_trial(results = res_list, which_pop=c("TRD", "PRD")[population], 
                                          ...
                                          hdi_perc = c(0.9,0.95), p_val = c(p_val_interim,p_val_final))
            ...
            } else if(nrow(res_list[[population]][[j]]$data) > n_int & res_list[[population]][[j]]$decision[1] == "none"){
            output <- make_decision_trial(results = res_list, which_pop=c("TRD", "PRD")[population], 
                                            which_admin=ways_of_administration[i], 
                                            which_treat=j, which_measure = 1,
